...
class modis_l1b:
    ...
    def __init__(self, \
        ...
        self.verbose    = verbose     # verbose tag
        self.keep_dims  = keep_dims   # flag; if false -> convert to 1D; true -> retain 2D/3D data
    ...
    def _get_250_500_attrs(self, hdf_dset_250, hdf_dset_500):
        ...
        return rad_off, rad_sca, ref_off, ref_sca
    ...
    def read(self, fname):
        ...
        # when resolution equals to 500 m
            ...
            rad_off, rad_sca, ref_off, ref_sca = self._get_250_500_attrs(raw0_250, raw0_500)
        ...
        # when resolution equals to 1000 m
            ...
            if self.f03 is not None:
                ...
                rad_off, rad_sca, ref_off, ref_sca = self._get_250_500_1km_attrs(raw0_250, raw0_500, raw0_1km_solar, raw0_1km_emissive)
        ...
        #/----------------------------------------------------------------------------\#
        wvl = np.zeros(len(self.bands), dtype='int16')
...
class modis_l2:
    ...
    def __init__(self, \
        ...
        self.extent     = extent      # specified region [westmost, eastmost, southmost, northmost]
        self.verbose    = verbose     # verbose tag
        self.keep_dims  = keep_dims   # flag; if true, gets the full 2D granule (essentially ignoring extent)
    ...
    def read(self, fname):
        ...
        ctp_ir      = f.select('Cloud_Phase_Infrared_1km')
#         cld_layer   = f.select('Cloud_Multi_Layer_Flag')
        ...
        ctt           = get_data_h4(ctt)
        ctp           = get_data_h4(ctp)
        ctp_ir        = get_data_h4(ctp_ir)

        cot0_data     = get_data_h4(cot0)
        cer0_data     = get_data_h4(cer0)
        cwp0_data     = get_data_h4(cwp0)

        cot_1621_data  = get_data_h4(cot_1621)
        cer_1621_data  = get_data_h4(cer_1621)
        cwp_1621_data  = get_data_h4(cwp_1621)

        cot_pcl_data  = get_data_h4(cot_pcl)
        cer_pcl_data  = get_data_h4(cer_pcl)
        cwp_pcl_data  = get_data_h4(cwp_pcl)

        cot_1621_pcl_data  = get_data_h4(cot_1621_pcl)
        cer_1621_pcl_data  = get_data_h4(cer_1621_pcl)
        cwp_1621_pcl_data  = get_data_h4(cwp_1621_pcl)

        # cot_uct = get_data_h4(cot_uct)
        # cer_uct = get_data_h4(cer_uct)
        # cwp_uct = get_data_h4(cwp_uct)

        if not self.keep_dims:
            cth           = cth[logic_5km]
            ctt           = ctt[logic_5km]
            ctp           = ctp[logic_1km]
            ctp_ir        = ctp_ir[logic_1km]

            cot0_data     = cot0_data[logic_1km]
            cer0_data     = cer0_data[logic_1km]
            cwp0_data     = cwp0_data[logic_1km]

            cot_1621_data  = cot_1621_data[logic_1km]
            cer_1621_data  = cer_1621_data[logic_1km]
            cwp_1621_data  = cwp_1621_data[logic_1km]

            cot_pcl_data  = cot_pcl_data[logic_1km]
            cer_pcl_data  = cer_pcl_data[logic_1km]
            cwp_pcl_data  = cwp_pcl_data[logic_1km]

            cot_1621_pcl_data  = cot_1621_pcl_data[logic_1km]
            cer_1621_pcl_data  = cer_1621_pcl_data[logic_1km]
            cwp_1621_pcl_data  = cwp_1621_pcl_data[logic_1km]
        ...
        if hasattr(self, 'data'):
            ...
            self.data['ctp_ir']   = dict(name='Cloud phase (IR)',                    data=np.hstack((self.data['ctp_ir']['data'], ctp_ir)),                units='N/A')
            ...
            self.data['ctp_ir']   = dict(name='Cloud phase (IR)',                    data=ctp_ir,                units='N/A')
...
class viirs_cldprop_l2:
    """
    Read VIIRS Level 2 cloud properties/mask file, e.g., CLDPROP_L2_VIIRS_SNPP.
    ...
    Output:
        ...
        self.data
                ...
                ['lat']
                ['cloud_mask_flag'] => 0: not determined, 1: determined
                ['fov_cat']          => 0: cloudy, 1: uncertain, 2: probably clear, 3: confident clear
                ['day_night_flag']  => 0: night, 1: day
        ...
        self.qa
                ...
                ...
                refer documentation below for more
    ...
    def __init__(self,
        ...
        self.f03               = f03       # geolocation class object created using the `viirs_03` reader
        self.extent            = extent    # specified region [westmost, eastmost, southmost, northmost]
        self.quality_assurance = quality_assurance # None or 0 = no QA; 1 = some QA; 2 = all QA
        self.keep_dims         = keep_dims # flag; if false -> convert to 1D; true -> retain 2D granule data
    ...
    def read_cop(self, fname):
        ...
        ctp_ir = f.groups['geophysical_data'].variables['Cloud_Phase_Cloud_Top_Properties'] # NOAA CLAVR-x AWG algorithm
        cth    = f.groups['geophysical_data'].variables['Cloud_Top_Height'] # NOAA CLAVR-x AWG algorithm
        ctt    = f.groups['geophysical_data'].variables['Cloud_Top_Temperature'] # NOAA CLAVR-x AWG algorithm
        ...
        ctp      = get_data_nc(ctp,    replace_fill_value=None)
        ctp_ir   = get_data_nc(ctp_ir, replace_fill_value=None)
        cth      = get_data_nc(cth,    replace_fill_value=None)
        ctt      = get_data_nc(ctt,    replace_fill_value=None)

        cot0_data     = get_data_nc(cot0)
        cer0_data     = get_data_nc(cer0)
        cwp0_data     = get_data_nc(cwp0)
        cot_pcl_data  = get_data_nc(cot_pcl)
        cer_pcl_data  = get_data_nc(cer_pcl)
        cwp_pcl_data  = get_data_nc(cwp_pcl)

        cot_1621_data     = get_data_nc(cot_1621)
        cer_1621_data     = get_data_nc(cer_1621)
        cwp_1621_data     = get_data_nc(cwp_1621)
        cot_1621_pcl_data = get_data_nc(cot_1621_pcl)
        cer_1621_pcl_data = get_data_nc(cer_1621_pcl)
        cwp_1621_pcl_data = get_data_nc(cwp_1621_pcl)

        if not self.keep_dims:
            ctp    = ctp[logic_extent]
            ctp_ir = ctp_ir[logic_extent]
            cth    = cth[logic_extent]
            ctt    = ctt[logic_extent]

            cot0_data     = cot0_data[logic_extent]
            cer0_data     = cer0_data[logic_extent]
            cwp0_data     = cwp0_data[logic_extent]
            cot_pcl_data  = cot_pcl_data[logic_extent]
            cer_pcl_data  = cer_pcl_data[logic_extent]
            cwp_pcl_data  = cwp_pcl_data[logic_extent]

            cot_1621_data     = cot_1621_data[logic_extent]
            cer_1621_data     = cer_1621_data[logic_extent]
            cwp_1621_data     = cwp_1621_data[logic_extent]
            cot_1621_pcl_data = cot_1621_pcl_data[logic_extent]
            cer_1621_pcl_data = cer_1621_pcl_data[logic_extent]
            cwp_1621_pcl_data = cwp_1621_pcl_data[logic_extent]
        ...
        if not self.keep_dims:
            ...
            cot = np.where(logic_pcl, cot_pcl_data, cot)
            cer = np.where(logic_pcl, cer_pcl_data, cer)
            cwp = np.where(logic_pcl, cwp_pcl_data, cwp)

            cot_1621 = np.where(logic_1621_pcl, cot_1621_pcl_data, cot_1621)
            cer_1621 = np.where(logic_1621_pcl, cer_1621_pcl_data, cer_1621)
            cwp_1621 = np.where(logic_1621_pcl, cwp_1621_pcl_data, cwp_1621)
        ...
        if hasattr(self, 'data'):
            ...
            self.data['lat']      = dict(name='Latitude',                            data=np.hstack((self.data['lat']['data'], lat)),                   units='degrees')
            self.data['ctp']      = dict(name='Cloud phase optical proprties',       data=np.hstack((self.data['ctp']['data'], ctp)),                   units='N/A')
            self.data['ctp_ir']   = dict(name='Cloud phase (IR) derived from NOAA CLAVR-X AWG algorithm', data=np.hstack((self.data['ctp_ir']['data'], ctp_ir)),      units='N/A')
            self.data['cth']      = dict(name='Cloud top height derived from NOAA CLAVR-X AWG algorithm', data=np.hstack((self.data['cth']['data'], cth)),                   units='m')
            self.data['ctt']      = dict(name='Cloud top temperature derived from NOAA CLAVR-X AWG algorithm', data=np.hstack((self.data['ctt']['data'], ctt)),            units='K')
            self.data['cot']      = dict(name='Cloud optical thickness',             data=np.hstack((self.data['cot']['data'], cot)),                   units='N/A')
            self.data['cer']      = dict(name='Cloud effective radius',              data=np.hstack((self.data['cer']['data'], cer)),                   units='micron')
            self.data['cwp']      = dict(name='Cloud water path',                    data=np.hstack((self.data['cwp']['data'], cwp)),                   units='g/m^2')
            self.data['cot_1621'] = dict(name='Cloud optical thickness (1621)',      data=np.hstack((self.data['cot_1621']['data'], cot_1621)),              units='N/A')
            ...
            self.data['ctp_ir']   = dict(name='Cloud phase (IR) derived from NOAA CLAVR-X AWG algorithm',      data=ctp_ir, units='N/A')
            self.data['cth']      = dict(name='Cloud top height derived from NOAA CLAVR-X AWG algorithm',      data=cth, units='m')
            self.data['ctt']      = dict(name='Cloud top temperature derived from NOAA CLAVR-X AWG algorithm', data=ctt, units='K')
            self.data['cot']      = dict(name='Cloud optical thickness',             data=cot,               units='N/A')
            self.data['cer']      = dict(name='Cloud effective radius',              data=cer,               units='micron')
            self.data['cwp']      = dict(name='Cloud water path',                    data=cwp,               units='g/m^2')
            self.data['cot_1621'] = dict(name='Cloud optical thickness (1621)',      data=cot_1621,          units='N/A')
